...
public class QuanLiBanDocPanel extends JPanel implements MouseListener {
	...
	public void init() {
		...
		JLDiaChi.setBounds(xMBD, yMBD + 80, 200, 50);
		JLDiaChi.setFont(font);
		add(JLDiaChi);

		JLCccd = new JLabel("CCCD      		         :");
		JLCccd.setBounds(xMBD, yMBD + 120, 200, 50);
		JLCccd.setFont(font);
		add(JLCccd);
		...
		JTDiaChi.setBounds(xMBD + 190, yMBD + 90, 300, 25);
		JTDiaChi.setFont(font);
		add(JTDiaChi);

		JTCccd = new JTextField();
		JTCccd.setBounds(xMBD + 190, yMBD + 130, 300, 25);
		JTCccd.setFont(font);
		add(JTCccd);
...
//		 add(JBChinhSua);
	...
	public void ShowBanDocTable() {
		...
		try {
			...
			String[] Column = { "Mã bạn đọc", "Tên bạn đọc", "Địa chỉ", "Cccd", "Số điện thoại", "Chức Vụ" }; // ADD COLUMN
	...
	public void ShowData() {
		...
		for (BanDoc bd : bddao.SelectAll()) { 				// ADD ROW // SelectAll TO GET DATABASE'S DATA /*CTRL FOR MORE*/
			Object[] row = { bd.getMaBanDoc(), bd.getTen(), bd.getDiaChi(), bd.getCccd(), bd.getSdt(), bd.getChucVu() };
			model.addRow(row);
		}
	}
	...
	public void Them() {
		try {
			if( BGChucVu.getSelection().getActionCommand()==null|| JTMaBanDoc.getText().equals("") || JTTen.getText().equals("") || JTCccd.getText().equals("") ) {
			...
			BanDoc bd = new BanDoc(MaBanDoc, Ten, DiaChi, Cccd, Sdt, ChucVu);
...
//		int row = JTbBanDoc.rowAtPoint(point);
		...
		JTDiaChi.setText((String) JTbBanDoc.getValueAt(row, 2));
		JTCccd.setText((String) JTbBanDoc.getValueAt(row, 3));
